...
\begin{eg}
    The standard pseudo-Euclidean vector space is the $n-$dimensional space $\mathbb{R}^{n-s,s}$, which consists of the vector space $\mathbb{R}^n$ with scalar product \[
        g_{ij} := \diag\{\underbrace{-1, \cdots, -1}_{s \text{ times}}, \underbrace{1, \cdots, 1}_{n-s \text{ times}}\} 
...
\begin{enumerate}
    ...
    \item The metric coefficients at $p$ are $\delta_{ij}$ in the Riemannian and $\pm \delta_{ij}$ in the semi-Riemannian case.
...
\begin{notation}
   ...
   \item The \textbf{Riemann tensor} is a $(0,4)-$tensor field defined by $\Rm:= \R^\flat=\langle \R(X,Y)Z,W  \rangle $.
