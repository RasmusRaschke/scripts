...
\end{split}
\end{equation}
ein Ringhomomorphismus ist. Wir brauchen gleich folgendes Lemma:
...
\begin{tikzcd}
    ...
    & \L \arrow[ul, hook, "\sub"] \arrow[ur, hook, "\iota"]& .
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
	\tilde{\psi}: \L[x] \arrow[r, "\iota_\ast"] & \E[x] \arrow[r, "\text{ev}_\alpha"] & \E \\
...
\begin{center}
\begin{tikzcd}
    \quotient{\L_i[x]}{(f_i(x))} \arrow[rr, hook, dashed, "\iota_{i+1}"] & & \E \\
    & \L_i \arrow[ul, hook, "\sub"] \arrow[ur, hook, "\iota_i"]& 
\end{tikzcd}
\end{center}
