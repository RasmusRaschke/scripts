...
\blue{
\begin{exa}\index{Quaternionen!Definition}
Wir definieren die {\em Quaternionengruppe} 
		\[
...
\begin{proof}
    ...
    N$. 
    \\ \red{Im vorigen Satz scheint etwas zu fehlen. Ist $\tilde{a}\tilde{b}\in abN$ gemeint?}\\
...
\begin{defi}\index{Kurze exakte Sequenz!von Gruppen}
Eine \emph{kurze exakte Sequenz}
    \[
        \{0\} \to N \overset{\phi}{\to} G \overset{\psi}{\to} H \to \{0\}
    \]
    von Gruppen besteht aus Gruppenhomomorphismen $\phi: N \to G$ und $\psi: G \to H$, so dass gilt:   \begin{enumerate}
        \item $\phi$ ist injektiv.
        \item $\Bild(\phi) = \Kern(\psi)$. 
        \item $\psi$ ist surjektiv. 
    \end{enumerate}
Wir sagen, dass eine kurze exakte Sequenz {\em zerfällt}, falls ein Homomorphismus $\sigma:H \to G$ mit $\psi \circ \sigma = \id_H$ existiert. Dann heißt $\sigma$ {\em Schnitt}.
\end{defi}
}
...
\begin{exas} \index{Konvexität!Definition}
    ...
    \begin{enumerate}\index{Reguläre Polyeder}
        \item Für $n \ge 2$, definieren wir das {\em reguläre $n$-gon}
            ...
            \]
            \red{Auf die Gefahr hin, die Diskussion aus der Vorlesung hervorzubringen: Ist das in der allgemeinen Vorstellung nicht eher ein reguläres Prisma?}
...
\blue{
\begin{exa}\index{Diedergruppe!Definition}
	Wir werden sehen, dass die Isometriegruppe des regelmäßigen $n$-gons die {\em Diedergruppe} $D_n$ ist. Die Gruppe besteht aus den $n$ Drehungen und $n$ Spiegelungen entlang der Symmetrieachsen des $n$-gons, in Matrixschreibweise gilt also:
	\[
		D_n = 
		\left\{ 
			\begin{pmatrix}
			...
			\cos \left( \frac{2 \pi k}{n}\right) & \sin \left( \frac{2 \pi k}{n}\right)\\
			\sin \left( \frac{2 \pi k}{n}\right) & -\cos \left( \frac{2 \pi k}{n}\right)
			\end{pmatrix} \mid 0 \leq k \leq n-1
		\right\}.
	\]
\end{exa}
}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $|{\SO}(P_n)| = n * 1 = n$.
        \item $|{\SO}(Z_n)| = 2n * 1 = 2n$. \red{Das macht zwar am Ende keinen Unterschied mehr, aber hier müsste, der obigen Erklärung folgend, $n*2$ stehen, oder? Wir haben $n$ rechteckige Randflächen, die eine $180^\circ$-Drehsymmetrie besitzen, also sind zwei Elemente im Stabilisator.}
        \item $|{\SO}(T)| = 4 * 3 = 12$.
...
\begin{figure}
\centering
\include{./graphics/platonic.tex}
\caption{Klassifikation der Drehgruppe $\SO(3,\RR)$.}
\end{figure}
...
\begin{exas}
    ...
    \begin{enumerate}
    		\item \index{Quotientenring!Restklassenring mod $n$} Sei $n \ge 2$ eine natürliche Zahl. Dann heißt der Ring $\ZZ/(n)$ der \emph{Restklassenring modulo $n$}.
    		\item \blue{\index{Quotientenkörper!Definition} Sei $R$ ein vom Nullring verschiedener Ring. Wir erklären auf $R \times (R \setminus \{0\})$ die Äquivalenzrelation $(a,b) \sim (c,d) :\iff ad=cb$ und schreiben $\frac{a}{b}$ für eine Äquivalenzklasse $[(a,b)]$. Auf Äquivalenzklassen definieren wir wie gewohnt Addition durch 
		\[
		...
		\]
    		Die Quotientenmenge $(R \setminus \{0\})^{-1}R:= R \times (R \setminus \{0\}) /_\sim$ heißt {\em Quotientenkörper} oder {\em Körper der Brüche} von $R$. Dies ist ein Spezialfall der in den Übungen untersuchten {\em Lokalisierung} von Ringen. Man überzeuge sich davon, dass tatsächlich ein Körper vorliegt (!).}
    \end{enumerate}
\end{exas}
...
\begin{figure}[H]
\centering
\include{./graphics/ring_implications.tex}
\caption{\blue{Dargestellt sind die wichtigsten Unterschiede der verschiedenen Ringe: Schwarze Pfeile gelten in allen Ringen, blaue Pfeile nur in Integritätsbereichen und rote Pfeile nur in Hauptidealringen.}}
\end{figure}
...
durchführen. \blue{Zentral ist dafür folgender Begriff:
\begin{defi}\index{Spektrum!Definition}
	Sei $R$ ein Ring. Die Menge
	\[
		\Spec (R) := \{ P \subseteq R \mid P \ \text{Primideal} \}
	\]
	heißt {\em Spektrum  von $R$}.
\end{defi}
}
...
\begin{thm}\index{Gaußscher Zahlenring!Spektrum}
    ...
    \blue{Das Spektrum von $\ZZ[i]$ besteht neben dem Nullideal aus:}
...
\begin{prob}[Eisenstein-Kriterium]\index{Eisenstein-Kriterium}
	Ein {\em primitives} Polynom
	\[
		f(X) = \sum_{i=0}^n a_i X^i
	\]
	ist irreduzibel über $\ZZ$ und $\QQ$, falls gilt: Es existiert eine Primzahl $p$, so dass:
		\begin{enumerate}[label=(\roman *)]
			\item $\forall i < n: \ p \mid a_i$,
			\item $p^2 \nmid a_0$ und
			\item $p \nmid a_n$.
    		\end{enumerate}
\end{prob}
...
\begin{exas}
    ...
    \item Für $R$-Moduln $M,N$ ist das kartesische Produkt $M \times N$ ein $R$-Modul mit
        ...
        \[
            M \oplus N := M \times N,
        \]
        genannt die \emph{direkte Summe} von $M$ und $N$.\index{Modul!direkte Summe}
    \end{enumerate}
\end{exas}
...
\begin{rem}
    ...
    ein {\em Schnitt von $g$} und wir sagen, die exakte Sequenz {\em spaltet} oder {\em \blue{zerfällt}}.
\end{rem}
...
\begin{lem}\index{Gradsatz für Körpertürme}
    ...
    \[
        [M:K] = [M:L][L:K].
    \]
    \blue{Ist die Formulierung nicht etwas zu stark? Die Formel gilt doch auch, wenn einer der Terme unendlich ist, und wird in der gängigen Literatur auch so angegeben.}
\end{lem}
\begin{proof}
    ...
    \[
        \sum_{i,j} \lambda_{i,j} l_i m_j = \sum_j \left( \sum_{i} \lambda_{i,j} l_i \right) m_j .
    \]
\end{proof}
...
\begin{exas}
    ...
    \begin{enumerate}
        ...
        \item Sei $K$ ein Körper und $K(X) = (K[X] \setminus \{0\})^{-1} K[X]$
            der Körper der Brüche, genannt \emph{Körper der rationalen Funktionen}.\index{Körper der rationalen Funktionen!Definition}
...
\begin{lem}
    ...
    eindeutig bestimmte Körpereinbettung
        ...
        \]
        so dass $\phi(X) = \alpha$. 
        \red{Hier hatten wir schon darüber gesprochen: $\phi$ sollte wahrscheinlich $\psi$ sein, korrekt? Selbiges im Beweis unten.}
\end{lem}
\begin{proof}
...
\begin{proof}[Beweis der Eindeutigkeit des Zerfällungskörpers.]
    ...
    \]
    \red{Ich bin hier etwas verwirrt hinsichtlich der Indizes: Erst zerlegen wir in $m$ Nullstellen über $L_n$, dann wenden wir $\phi_m$ auf einmal auf $n$ Nullstellen über $E$ an. Ist hier eventuell einmal $n$ und $m$ durcheinandergeraten?}
\end{proof}
...
\begin{proof}
    ...
    \]
    verschiedene Einbettungen $L_n \hra L_n = E$ \red{Hier müsste $L_{n-1} \hra L_n=E$ stehen, oder?}, die, mit dem Argument im
...
\blue{
\begin{defi}\index{Charakteristik!Definition}
	\label{defi:charakteristik}
	Sei $R$ ein Ring. Die {\em Charakteristik} $\charac(R)$ von $R$ ist die eindeutig bestimmte natürliche Zahl $n$, sodass $\ZZ / n \ZZ$ der Kern des unitalen Ringhomomorphismus
		\[
			\chi: \ZZ \to R, \ n \mapsto n \cdot 1_R
		\]
	ist.\\
	Äquivalent ist die Charakteristik die kleinste natürliche Zahl $n \geq 1$, sodass 
	\[
		\sum_{i=1}^n 1_R = 0_R
	\]
	gilt, falls diese Gleichung für ein $n$ in $R$ lösbar ist. Andernfalls setzt man $\charac(R)=0$. 
\end{defi}
Der Vorteil der ersten Definition ist, dass man keine Fallunterscheidung vornehmen muss.}
...
\begin{proof}
    ...
    zyklisch \red{Die Anmerkung ist von Dir: (todo: include proof!)}
\end{proof}
...
\begin{defi}\index{Ordnung!Totalordnung}
...
\begin{defi}\index{Ordnung!Wohlordnung}
...
\begin{prop}[Wohlordnungssatz]\index{Wohlordnungssatz}
...
\begin{prop}[Lemma von Zorn]\index{Lemma von Zorn}
