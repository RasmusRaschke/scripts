...
\end{equation}
die \textbf{Ordnung von} $g$ und $\langle g \rangle$ \textbf{zyklisch} der Ordnung $\ord (g)$.
\end{definition}
...
Sei $G$ eine Gruppe und $H \leq G$ eine Untergruppen. Dann ist die \textbf{Linksnebenklasse von} $H$ \textbf{bezüglich} $g$ als 
\begin{equation}
gH := \{gh \, | \, h \in H\}
\end{equation}
und die \textbf{Rechtsnebenklasse von} $H$ \textbf{bezüglich} $g$ als 
\begin{equation}
Hg := \{hg \, | \, h \in H \}
\end{equation}
definiert.
...
\end{satz}
\begin{beweis}
Dem Leser als Übung überlassen. Es ist insbesondere auch zu zeigen, dass $\sim$ tatsächlich eine Äquivalenzrelation definiert.
\end{beweis}
...
\begin{beweis}
\begin{enumerate}
\item Zunächst muss die Wohldefiniertheit der Verknüpfung bewiesen werden. Seien $\atil \in aN$ und $\btil \in bN$ Vertreter der Nebenklassen $aN$ und $bN$ $(\iff \atil N = aN)$. Dann existieren $m,n \in N$ mit $\atil = am$ und $\btil = bn$. Nun gilt
\begin{equation}
\atil \cdot \btil = am \cdot bn = ab \cdot \underbrace{b^{-1} mb}_{N \trianglelefteq G \implies \in N} n \in N,
\end{equation}
...
\end{satz}
\begin{beweis}
Zunächst ist Wohldefiniertheit zu zeigen. Für $\gtil \in gN$, also $\gtil = gn$ für $n \in \ker (\phi)$, gilt:
\begin{equation}
\phi( \gtil) = \phi(gn) = \phi(g)\underbrace{\phi(n)}_{=1} = \phi(g),
\end{equation}
...
\end{beispiel}
\begin{satz}{Korollar aus Satz \ref{homomorphiesatz}}{korollarhomosatz}
...
\end{enumerate}
Dann sagen wir, dass $G$ auf $M$ \textbf{operiert} und schreiben $G\circlearrowright M$.
\end{definition}
\begin{beispiele}
\begin{enumerate}
...
Für Operationen $G\circlearrowright M$ und $G\circlearrowright N$ heißt eine Abbildung (von Mengen) $f: M \to N$ $G$-\textbf{äquivariant}, falls für alle $g \in G$ und $x \in M$ gilt: 
\begin{equation}
f(g.x) = g.f(x).
\end{equation}
\end{definition}
\begin{definition}{Bahnen}{bahnen}
Sei $G \circlearrowright M$ eine Operation von $G$ auf $M$. Die Relation
\begin{equation}
...
G.x := \{g.x | g \in G\}
\end{equation}
für $x \in M$, die \textbf{Bahnen} von $x$ unter $G \circlearrowright M$ genannt werden. Die Quotientenmenge 
\begin{equation}
\invquotient{M}{G} := \quotient{M}{\sim_G}
\end{equation}
heißt \textbf{Bahnenraum von} $G \circlearrowright M$.
\end{definition}
\begin{beweis}
...
Sei $G \circlearrowright M$ eine Operation. 
\begin{enumerate}[(i)]
...
\item Ein Punkt $x \in M$ heißt \textbf{Fixpunkt von} $G \circlearrowright M$, falls $G_x = G$. Die Menge aller Fixpunkte wird mit 
\begin{equation}
M^G \sub M
\end{equation}
bezeichnet.
\item Die Operation $G \circlearrowright M$ heißt \textbf{transitiv}, falls für jedes $x \in M$ gilt, dass $G.x = M$ ist, also genau eine Bahn existiert.
\end{enumerate}
\end{definition}
\begin{beispiel}
...
Sei $G\circlearrowright M$ eine Operation auf $M$ und $x \in M$. Dann definiert 
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
eine bijektive, $G$-äquivariante Abbildung, wobei $G \circlearrowright G.x$ durch Einschränkung von $G\circlearrowright M$ gegeben ist. Insbesondere gilt die \textbf{Bahnformel}
\begin{equation}
|G.x| = (G : G_x).
\end{equation}
Eine Wirkung $G \circlearrowright \quotient{G}{G_x} = \{gG_x|g \in G\}$ erhält man durch $g' . gG_x:= g'g.G_x$.
\end{satz}